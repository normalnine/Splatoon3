Shader "WaterSplashEffects/VertexAnimationParticle" {
	Properties {
		...
		_timeCount("Time Count", Float) = 0.0
	...
	SubShader {
		...
		#define UNITY_PARTICLE_INSTANCE_DATA WSEParticleInstanceData
		#define UNITY_PARTICLE_INSTANCE_DATA_NO_ANIM_FRAME
		struct WSEParticleInstanceData
		{
			float3x4 transform;
			uint color;
			float lifetime;
		...
		void vert(inout appdata_custom v){
			...
			float t = _Time.y;
#endif
			float timeInFrames = (ceil(frac(-t * 1.0f) * _numOfFrames))/_numOfFrames;
